...
{
	//ゲームオーバー
	if (m_state == enGameState_Result && g_pad[0]->IsTrigger(enButtonA))
	{
	...
	//ステート
	if (m_state == enGameState_Container && m_isContainerNewGO == true)
	{
		ContainerNewGO();
		m_isContainerNewGO = false;
	}
	else if (m_state == enGameState_InGame && m_isInGameNewGO == true)
	{
		InGameNewGO();
		m_isInGameNewGO = false;
	}
}
...
{
	EffectEngine::GetInstance()->ResistEffect(enEffectNumber_Barrier, u"Assets/effect/Barrier.efk");
...
void Game::InitSound()
{
	...
	g_soundEngine->ResistWaveFileBank(enSoundNumber_ContainerDoor, "Assets/sound/ContainerDoor.wav");
	g_soundEngine->ResistWaveFileBank(enSoundNumber_ContainerShipMove, "Assets/sound/ContainerShipMove.wav");
